	...
	public static class DERMSConverter
	{
        ...
        public static void PopulateEnergyStorageProperties(EnergyStorage energyStorage, ResourceDescription rd, ImportHelper importHelper, TransformAndLoadReport report)
        {
            if ((energyStorage != null) && (rd != null))
            {
                ...
                {
                    report.Report.Append($"EnergyStorage with id = {energyStorage.MRID} will have default value of property \"EnergyStorageState\" = {Core.Common.AbstractModel.EnergyStorageState.Idle}\n");
                    rd.AddProperty(new Property(ModelCode.ENERGYSTORAGE_STATE, (short)Core.Common.AbstractModel.EnergyStorageState.Idle));
                }
        ...
        private static Core.Common.AbstractModel.SignalDirection SignalDirectionMap(DERMS.SignalDirection signalDirection)
        {
            switch (signalDirection)
            {
                case DERMS.SignalDirection.Read:
                    return Core.Common.AbstractModel.SignalDirection.Read;
                case DERMS.SignalDirection.ReadWrite:
                    return Core.Common.AbstractModel.SignalDirection.ReadWrite;
                case DERMS.SignalDirection.Write:
                    return Core.Common.AbstractModel.SignalDirection.Write;
                default:
                    return 0;
            }
        }
        ...
        private static Core.Common.AbstractModel.MeasurementType MeasurementTypeMap(DERMS.MeasurementType measurementType)
        {
            switch (measurementType)
            {
                case DERMS.MeasurementType.ActiveEnergy:
                    return Core.Common.AbstractModel.MeasurementType.ActiveEnergy;
                case DERMS.MeasurementType.ActivePower:
                    return Core.Common.AbstractModel.MeasurementType.ActivePower;
                case DERMS.MeasurementType.Discrete:
                    return Core.Common.AbstractModel.MeasurementType.Discrete;
                case DERMS.MeasurementType.Humidity:
                    return Core.Common.AbstractModel.MeasurementType.Humidity;
                case DERMS.MeasurementType.Money:
                    return Core.Common.AbstractModel.MeasurementType.Money;
                case DERMS.MeasurementType.Percent:
                    return Core.Common.AbstractModel.MeasurementType.Percent;
                case DERMS.MeasurementType.SkyCover:
                    return Core.Common.AbstractModel.MeasurementType.SkyCover;
                case DERMS.MeasurementType.Status:
                    return Core.Common.AbstractModel.MeasurementType.Status;
                case DERMS.MeasurementType.SunshineMinutes:
                    return Core.Common.AbstractModel.MeasurementType.SunshineMinutes;
                case DERMS.MeasurementType.Temperature:
                    return Core.Common.AbstractModel.MeasurementType.Temperature;
                case DERMS.MeasurementType.Time:
                    return Core.Common.AbstractModel.MeasurementType.Time;
